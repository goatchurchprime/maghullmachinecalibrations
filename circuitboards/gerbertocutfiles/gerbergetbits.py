...
def GetEdgeRange(fname):
    ...
    poly = g.solid_geometry
    conts = ExtractContoursFromPoly(poly, 0, 0)
    minx = min(min(x  for x,y in cont)  for cont in conts)
    miny = min(min(y  for x,y in cont)  for cont in conts)
    maxx = max(max(x  for x,y in cont)  for cont in conts)
    maxy = max(max(y  for x,y in cont)  for cont in conts)
