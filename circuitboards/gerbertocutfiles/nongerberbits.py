...
def sendgcodetotwisted(ngcF, ipnumber):
    import httplib
    h = httplib.HTTPConnection(ipnumber+':9003')
    headers = {"Content-type": "application/x-www-form-urlencoded", "Accept": "text/plain"}
    h.request('POST', '/cgipipe/savengc.py?autorun=no', open(ngcF).read(), headers)
    r = h.getresponse()
    print(r.read())
...
def FragmentContour(cont, maxsegmentlength):
    pp = cont[0]
    rcont = [ pp ]
    for p in cont[1:]:
        l = math.sqrt((pp[0] - p[0])**2 + (pp[1] - p[1])**2)
        if l > maxsegmentlength:
            n = int(l/maxsegmentlength)+1
            for i in range(1, n):
                lam = i*1.0/n
                rcont.append((pp[0]*(1-lam)+p[0]*lam, pp[1]*(1-lam)+p[1]*lam))
        rcont.append(p)
        pp = p
    return rcont
